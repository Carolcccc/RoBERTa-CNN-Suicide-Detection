...
try:
    ...
    HAS_WANDB = False
    print("Warning: wandb not installed. Run with --no-wandb flag.")
...
def train(model, train_loader, val_loader, optimizer, loss_fn, device, epochs, use_wandb=True):
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs}, Train Loss: {train_loss/len(train_loader):.4f}, Train Accuracy: {correct_preds/total_preds:.4f}")
...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='RoBERTa-CNN Suicide Detection Model Training')
    parser.add_argument('--data-path', type=str, default='Suicide_Dataset_sample.csv',
                        help='path to dataset CSV file (default: Suicide_Dataset_sample.csv)')
    ...
    if 'class' in data.columns:
        ...
        print("Error: CSV must contain 'class' or 'label' column")
        sys.exit(1)
    ...
    print(f"Total samples after augmentation: {len(combined_data)}")
    ...
    train(model, train_loader, val_loader, optimizer, loss_fn, device, epochs, use_wandb=use_wandb)
    ...
    if args.save_model:
        model_path = 'roberta_cnn_model.pt'
